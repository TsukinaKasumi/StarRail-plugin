...
function getdamages_num(skill_type, data) {
    ...
    if (skill_type == 'Normal') {
        skill_multiplier = getskilllevelnum(String(data.avatarId), data.skillTree, 'basic_atk', skill_type);
        if (String(data.avatarId) == '1004' && data.rank >= 1) {
            skill_multiplier = skill_multiplier + skill_multiplier * 0.5;
        }
    }
    else if (skill_type == 'BPSkill') {
        skill_multiplier = getskilllevelnum(String(data.avatarId), data.skillTree, 'skill', skill_type);
        if (String(data.avatarId) == '1004' && data.rank >= 1) {
            skill_multiplier = skill_multiplier + skill_multiplier * 0.8;
        }
    }
    else if (skill_type == 'Ultra') {
        skill_multiplier = getskilllevelnum(String(data.avatarId), data.skillTree, 'ultimate', skill_type);
        if (String(data.avatarId) == '1006' && data.rank >= 4) {
            skill_multiplier = skill_multiplier + 1;
        }
    }
    else if (skill_type == 'Talent') {
        skill_multiplier = getskilllevelnum(String(data.avatarId), data.skillTree, 'talent', skill_type);
        ...
        if (String(data.avatarId) == '1107') {
            let skill_multiplier_1 = getskilllevelnum(String(data.avatarId), data.skillTree, 'talent', skill_info[3]);
            let skill_multiplier_2 = getskilllevelnum(String(data.avatarId), data.skillTree, 'ultimate', skill_info[3]);
            skill_multiplier = skill_multiplier_1 + skill_multiplier_2;
            skill_type = 'Talent';
        }
        else if (String(data.avatarId) == '1213' ||
            String(data.avatarId) == '1201') {
            skill_multiplier = getskilllevelnum(String(data.avatarId), data.skillTree, 'basic_atk', skill_info[3]);
            skill_type = 'Normal';
        }
        else if (String(data.avatarId) == '1005') {
            skill_multiplier = getskilllevelnum(String(data.avatarId), data.skillTree, 'ultimate', skill_info[3]);
            if (data.rank >= 6) {
            ...
            skill_multiplier = getskilllevelnum(String(data.avatarId), data.skillTree, 'skill', skill_info[3]);
            skill_type = 'BPSkill';
        }
        else if (String(data.avatarId) == '1112') {
            let skill_multiplier_1 = getskilllevelnum(String(data.avatarId), data.skillTree, 'skill', 'BPSkill');
            let skill_multiplier_2 = getskilllevelnum(String(data.avatarId), data.skillTree, 'ultimate', skill_info[3]);
            skill_multiplier = skill_multiplier_1 + skill_multiplier_2;
            skill_type = 'Talent';
        }
    ...
    for (let i = 0; i < attribute_attrkey.length; i++) {
        ...
        if (attr.search('AttackAddedRatio') != -1) {
            ...
            if (skill_name.search(skill_type) != -1 ||
                skill_name.search(skill_info[3]) != -1) {
    ...
    if (skill_info[0] == 'attack') {
        ...
        if (String(data.avatarId) == '1004') {
            ...
            let multiplier_add = getskilllevelnum(String(data.avatarId), data.skillTree, 'talent', 'Talent');
        ...
        if (['1205', '1208'].includes(String(data.avatarId))) {
            ...
            if (skill_type == 'Normal') {
                if (String(data.avatarId) == '1208') {
                    hp_multiplier = getskilllevelnum(String(data.avatarId), data.skillTree, 'basic_atk', 'Normal_HP');
                }
            }
            else if (skill_type == 'Normal1') {
                hp_multiplier = getskilllevelnum(String(data.avatarId), data.skillTree, 'basic_atk', 'Normal1_HP');
                skill_type = 'Normal';
            }
            else if (skill_type == 'Ultra') {
                hp_multiplier = getskilllevelnum(String(data.avatarId), data.skillTree, 'ultimate', 'Ultra_HP');
                ...
                hp_multiplier = getskilllevelnum(String(data.avatarId), data.skillTree, 'talent', 'Talent_HP');
        ...
        for (let i = 0; i < merged_attrkey.length; i++) {
            ...
            if (attr.search('ResistancePenetration') != -1) {
                ...
                if (attr_name.search(data.damage_type) != -1 ||
                    attr_name.search('AllDamage') != -1) {
                ...
                if (attr_name.search('_') != -1) {
                    ...
                    if ((skill_name.search(skill_type) != -1 ||
                        skill_name.search(skill_info[3]) != -1) &&
                        (skillattr_name.search(data.damage_type) != -1 ||
                            skillattr_name.search('AllDamage') != -1)) {
        ...
        for (let i = 0; i < merged_attrkey.length; i++) {
            ...
            if (attr.search('DmgAdd') != -1) {
                ...
                if (attr_name.search(skill_type) != -1 ||
                    attr_name.search(skill_info[3]) != -1) {
        ...
        for (let i = 1; i <= skill_info[2]; i++) {
            ...
            if (String(data.avatarId) == '1213') {
                injury_add = getskilllevelnum(String(data.avatarId), data.skillTree, 'talent', 'Talent');
                critical_damage_add = getskilllevelnum(String(data.avatarId), data.skillTree, 'skill', 'BPSkill');
